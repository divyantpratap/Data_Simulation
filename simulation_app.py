...
def run_simulation(donor_appliance, hours_of_shift, acceptor_uuid_file, randomization, donor_uuid_file, s3_disagg_output_location, raw_data_file, heatmap_folder):
    ...
    def generate_shift_values(num_instances, randomization, points_to_shift, sigma):
        if randomization == '1':
            ...
            shift_values = shift_values.round().astype(int)
    ...
    def check_timestamp_more_than_one_months(start_timestamp, last_timestamp):
        """
        ...
        one_months_in_seconds = 1 * 30 * 24 * 60 * 60

        # Check if the difference is within six months
        if timestamp_difference >= one_months_in_seconds:
            return True
        else:
            return False
    ...
    def get_metadata(env, uuid,access_token):
        ...
        protocol = env_properties['protocol']
        primary_api = env_properties['primary']
            
        endpoint = f"{protocol}{primary_api}/meta/users/{uuid}/homes/1"
        ...
        if response.status_code == 200:
            try:
                ...
                if not os.path.exists(folder_path):
                    ...
                    # Ensure the keys in metadata have the required fields and update if necessary
                metadata['country'] = metadata.get('country', None)
    ...
    def create_different_year_user(disagg_output, raw_energy_data):
        # Create a copy of the disagg_output dataframe
        updated_disagg_output = disagg_output.copy()

        # Convert the timestamps to datetime objects
        updated_disagg_output['timestamp'] = pd.to_datetime(updated_disagg_output['timestamp'], unit='s')
        raw_energy_data['timestamp'] = pd.to_datetime(raw_energy_data['timestamp'], unit='s')

        # Calculate sampling rate
        sampling_rate_s = updated_disagg_output['timestamp'][0:2].diff().iloc[1]
        sampling_rate = int(sampling_rate_s.total_seconds() // 60)  # Convert sampling_rate to minutes

        # Create 'time_index' for both dataframes
        updated_disagg_output['time_index'] = updated_disagg_output['timestamp'].dt.month*1000000 + updated_disagg_output['timestamp'].dt.day*10000 + updated_disagg_output['timestamp'].dt.hour*100 + (updated_disagg_output['timestamp'].dt.minute // sampling_rate) * sampling_rate
        raw_energy_data['time_index'] = raw_energy_data['timestamp'].dt.month*1000000 + raw_energy_data['timestamp'].dt.day*10000 + raw_energy_data['timestamp'].dt.hour*100 + (raw_energy_data['timestamp'].dt.minute // sampling_rate) * sampling_rate

        # Merge the dataframes on the 'time_index' column
        merged_data = pd.merge(updated_disagg_output, raw_energy_data, on='time_index', how='right')

        # Additional operations
        if 'appliance_output' in merged_data.columns:
            merged_data['total_output'] = merged_data['appliance_output'] + merged_data['value']
            merged_data.drop(['appliance_output', 'value', 'duration'], axis=1, inplace=True)
            merged_data.rename(columns={'total_output': 'value'}, inplace=True)
        else:
            merged_data['total_output'] = merged_data.get('value_x', 0) + merged_data.get('value_y', 0) # use .get() to avoid KeyError
            columns_to_drop = ['value_x', 'value_y', 'datetimenew', 'date', 'time']
            for col in columns_to_drop:
                if col in merged_data.columns:
                    merged_data.drop(col, axis=1, inplace=True)
        ...
        # Drop the 'time_index' and 'timestamp_x' columns from the merged data
        merged_data.drop(['time_index', 'timestamp_x'], axis=1, inplace=True)
    ...
    def downsample(donor_data, downsample_factor, donor_sampling_rate):
        ...
        # Create a copy of the donor_data DataFrame
        donor_data_copy = donor_data.copy()

        # Convert the epoch timestamp to a datetime object
        donor_data_copy['timestamp'] = pd.to_datetime(donor_data_copy['timestamp'], unit='s')

        # Set the timestamp column as the index
        donor_data_copy.set_index('timestamp', inplace=True)

        # Downsample the data using the specified downsample_factor
        downsampled_data = donor_data_copy.resample(f"{downsample_factor * donor_sampling_rate}S").sum()
    ...
    def generate_heatmap(shifted_disagg_output, df_out ,raw_energy_data, donor_uuid, uuid, acceptor_sampling_rate, timezone,heatmap_folder,year_change,donor_sampling_rate):
        """
        ...
        if year_change == 0:
            ...
            df_out.rename(columns={'appliance_output': 'value'}, inplace=True)
            create_heatmap(df_out, "Donor User Appliance Usage", timezone, ax=axs[0])
    ...
    def process_raw_data(raw_file,raw_data, uuid, donor_uuid, df_out, shifted_disagg_output, timezone, env,donor_sampling_rate,year_change):
        """
        ...
        with open(raw_file, "w") as file:
            ...
            if compareSamplingRate(donor_sampling_rate, acceptor_sampling_rate) == 0:
                generate_heatmap(shifted_disagg_output, df_out, raw_energy_data, donor_uuid, uuid, acceptor_sampling_rate, timezone,heatmap_folder,year_change,donor_sampling_rate)

            elif donor_sampling_rate < acceptor_sampling_rate:
                downsample_factor = int(acceptor_sampling_rate // donor_sampling_rate)
                shifted_disagg_data_downsampled = downsample(shifted_disagg_output, downsample_factor,donor_sampling_rate)
                print(shifted_disagg_data_downsampled.head())
                generate_heatmap(shifted_disagg_data_downsampled, df_out,raw_energy_data, donor_uuid, uuid, acceptor_sampling_rate, timezone,heatmap_folder,year_change,donor_sampling_rate)
                del shifted_disagg_data_downsampled
    ...
    if s3_disagg_output_location is None or s3_disagg_output_location == '':
        ...
        if donor_appliance == "ev":
            s3_disagg_output_location = "s3://bidgely-ds/NON_ARCHIVE/Data_Simulation/donor/ev"
        elif donor_appliance == "pp":
            s3_disagg_output_location = "s3://bidgely-ds/NON_ARCHIVE/Data_Simulation/donor/pp"
        elif donor_appliance == "timed_wh":
            s3_disagg_output_location = "s3://bidgely-ds/NON_ARCHIVE/Data_Simulation/donor/timed_wh"
        elif donor_appliance == "ac":
            s3_disagg_output_location = "s3://bidgely-ds/NON_ARCHIVE/Data_Simulation/donor/ac"
        elif donor_appliance == "sh":
            s3_disagg_output_location = "s3://bidgely-ds/NON_ARCHIVE/Data_Simulation/donor/sh"
    ...
    for index, row in donor_uuids.iterrows():
        ...
        donor_timezone, donor_country = get_metadata(donor_env, donor_uuid, donor_access_token) 
        ...
        try:
            if not os.path.exists('uuid_mapping.csv'):
                ...
                with open('uuid_mapping.csv', mode='w', newline='') as csv_file:
                    fieldnames = ['donor_uuid', 'acceptor_uuid', 'env', 'timezone','country','status']
                    writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
                    writer.writeheader()
                    
            s3_path = f"{s3_disagg_output_location}/{donor_uuid}_tou.csv"
            ...
            shifted_disagg_output = shifting_disagg_data(df_x, hours_of_shift, randomization,donor_sampling_rate)
            ...
            for index, row in acceptor_uuids.iterrows():
                ...
                if donor_country == acceptor_country:    
                    ...
                    if raw_data and check_timestamp_more_than_one_months(start_timestamp, last_timestamp):
                        #print(f"Debug: raw_data for user {uuid}: {raw_data}")
                            
                        acceptor_delta = last_timestamp-start_timestamp
                        print(acceptor_delta) 
                        if check_timestamp_more_than_one_months(start_timestamp, last_timestamp):
                            year_change = 0 
                            process_raw_data(raw_file,raw_data, uuid, donor_uuid, disagg_output_home,shifted_disagg_output,  timezone, env,donor_sampling_rate,year_change=0)
                            
                            
                    elif raw_data ==[] or raw_data is None or not check_timestamp_more_than_one_months(start_timestamp, last_timestamp):
                        #print("Changing different year data")
                        last_billing_cycle = get_last_billing_cycle(uuid)
                        end_epoch_timestamp = last_billing_cycle["value"]
                        
                        one_year_in_seconds = 365 * 24 * 60 * 60
                        start_epoch_timestamp = end_epoch_timestamp - one_year_in_seconds
                        ...
                        donor_end_time = end
                        donor_start_time = end - one_year_in_seconds
                        print(donor_start_time,donor_end_time)

                        # Filter the DataFrame to keep only the rows with a timestamp between the start and end timestamps
                        filtered_disagg_output_home = disagg_output_home[(disagg_output_home['timestamp'] >= donor_start_time) & (disagg_output_home['timestamp'] <= donor_end_time)]
                        shifted_filtered_disagg_output = shifted_disagg_output[(shifted_disagg_output['timestamp'] >= donor_start_time) & (shifted_disagg_output['timestamp'] <= end_timestamp)]
                            
                        if raw_data:
                            year_change =1
                            process_raw_data(raw_file, raw_data, uuid, donor_uuid, filtered_disagg_output_home,
                                                shifted_filtered_disagg_output, timezone, env,donor_sampling_rate,year_change=1)
                            del filtered_disagg_output_home
                            del shifted_filtered_disagg_output
                        else:
                    ...
                    print(f"Different countries for {donor_uuid} and {uuid}")
                ...
                with open('uuid_mapping.csv', mode='a', newline='') as csv_file:
                    ...
                    file_path = f"./created_user_raw_data/{uuid}_{donor_uuid}.csv"
                    if os.path.exists(file_path):
                        status = "Simulation Done"
                    else:
                        status = "No Simulation"
                    writer.writerow({'donor_uuid': donor_uuid, 'acceptor_uuid': uuid, 'env': env, 'timezone': timezone, 'country':acceptor_country, 'status': status}) 
                
        except Exception as e:
            print(f"Error in simulating user at {donor_uuid}: {e}")
            logging.error(f"Error in simulating user at {donor_uuid}: {e}")
            completed_uuids += 1
